...
$sql = "SELECT id, cantidad_cedear, precio_cedear, fecha_cedear FROM cedear WHERE ticker_cedear = ? AND cliente_id = ?";
$stmt = $conn->prepare($sql);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($precio_cedear_compra_original > $nuevo_precio_compra) {
        $diferencia = $precio_cedear_compra_original - $nuevo_precio_compra;
        $nuevo_efectivo = $efectivo + $diferencia;
        $actualizar_efectivo = true;
    } else {
        $diferencia = $nuevo_precio_compra - $precio_cedear_compra_original;
    ...
    if ($actualizar_efectivo) {
        ...
        $sql = "UPDATE cedear SET cantidad_cedear = ?, precio_cedear = ?, fecha_cedear = ? WHERE id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("dssi", $nuevo_cantidad, $nuevo_precio, $fecha_cedear, $cedear_id);
        $stmt->execute();
        $stmt->close();
...
<body>
                    ...
                    <div class="row mb-3 align-items-center">
                        ...
                        <div class="col-sm-10">
                            <div class="input-group">
                                ...
                                <input type="text" step="0.01" class="form-control" id="precio" name="precio" value="<?php echo htmlspecialchars($precio_cedear); ?>" placeholder="0,00" required>
                            </div>
                        </div>
                    </div>
                    <!-- Fecha -->
    ...
    <script src="../js/tickers_cedears.js"></script>
    <!-- FIN JS -->
</body>
