...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $cliente_id = $_POST['cliente_id'];
    $ticker = $_POST['ticker'];
    $cantidad = floatval(str_replace(',', '.', str_replace('.', '', $_POST['cantidad'])));
    $fecha_compra = DateTime::createFromFormat('d-m-Y', $_POST['fecha_compra'])->format('Y-m-d');
    $precio_compra = floatval(str_replace(',', '.', str_replace('.', '', $_POST['precio_compra'])));
    $ccl_compra = floatval(str_replace(',', '.', str_replace('.', '', $_POST['ccl_compra'])));
    $fecha_venta = DateTime::createFromFormat('d-m-Y', $_POST['fecha_venta'])->format('Y-m-d');
    $precio_venta = floatval(str_replace(',', '.', str_replace('.', '', $_POST['precio_venta'])));
    $ccl_venta = floatval(str_replace(',', '.', str_replace('.', '', $_POST['ccl_venta'])));

    if ($cantidad <= $cantidad_mostrar) {
        // Actualizar saldo
        $total_venta = $cantidad * $precio_venta;
        $nuevo_saldo = $saldo_en_pesos + $total_venta;
        actualizarSaldo($cliente_id, $nuevo_saldo);
        ...
        $sql_update_accion = "UPDATE acciones SET cantidad = ? WHERE cliente_id = ? AND ticker = ?";
        $stmt = $conn->prepare($sql_update_accion);
        ...
        // Manejar errores de cantidad ingresada
        if ($cantidad == $cantidad_total) {
            echo "<script>
                if (confirm('La cantidad de acciones ingresadas es igual a la cantidad de acciones en tenencia.\n¿Querés hacer una venta total?')) {
                    window.location.href = 'venta_total_acciones.php?cliente_id={$cliente_id}&ticker={$ticker}';
                } else {
            ...
            echo "<script>
                alert('La cantidad de acciones ingresadas es mayor a la cantidad de acciones en tenencia.');
                document.getElementById('cantidad').focus();
            </script>";
        }
    }
