$(document).ready(function() {
    function updateDropdownPosition() {
        ...
        var dropdown = $('#tickerDropdown_cedears');
    ...
    $(document).on('click', '#tickerDropdown_cedears .dropdown-item', function(e) {
        e.preventDefault();
        var text = $(this).text().split(' - ')[0];
        $('#ticker').val(text);
        $('#tickerDropdown_cedears').empty().hide();
    });

    $('#ticker').on('keydown', function(e) {
        var dropdownItems = $('#tickerDropdown_cedears .dropdown-item');
