...
$ticker_cedear = isset($_GET['ticker']) ? $_GET['ticker'] : '';
...
// Consulta para obtener los valores de cantidad, precio y id de la tabla cedear
$sql = "SELECT id, cantidad, precio, fecha FROM cedear WHERE ticker = ? AND cliente_id = ?";
$stmt = $conn->prepare($sql);
$stmt->bind_param("si", $ticker_cedear, $cliente_id);
$stmt->execute();
$stmt->bind_result($id_cedear, $cantidad_cedear, $precio_cedear, $fecha_cedear);
$stmt->fetch();
$stmt->close();
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $cedear_id = $_POST['cedear_id'];  // Se obtiene el ID del cedear
    $nuevo_cantidad_cedear = $_POST['cantidad'];
    $nuevo_precio_cedear = $_POST['precio'];
    $fecha_cedear = $_POST['fecha'];

    // Calcular el costo de compra
    $nuevo_precio_compra_cedear = $nuevo_cantidad_cedear * $nuevo_precio_cedear;
    ...
    $precio_cedear_compra_original = $cantidad_cedear * $precio_cedear;
    if ($precio_cedear_compra_original > $nuevo_precio_compra_cedear) {
        $diferencia = $precio_cedear_compra_original - $nuevo_precio_compra_cedear;
        $nuevo_efectivo = $efectivo + $diferencia;
        $actualizar_efectivo = true;
    } else {
        $diferencia = $nuevo_precio_compra_cedear - $precio_cedear_compra_original;
    ...
    if ($actualizar_efectivo) {
        ...
        // Actualizar solo el cedear específico con el ID correspondiente
        $sql = "UPDATE cedear SET cantidad = ?, precio = ?, fecha = ? WHERE id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("dssi", $nuevo_cantidad_cedear, $nuevo_precio_cedear, $fecha_cedear, $cedear_id);
        $stmt->execute();
        $stmt->close();
    ...
    <div class="row mx-2 mt-navbar">
        ...
        <div class="col-6 text-center">
            <div class="container-fluid my-4 efectivo">
                <h5 class="me-2 cartera titulo-botones mb-4">Editar los datos de <?php echo htmlspecialchars($ticker_cedear); ?></h5>
                <?php if ($error_msg): ?>
                ...
                <form method="POST" action="">
                    <input type="hidden" name="cedear_id" value="<?php echo htmlspecialchars($id_cedear); ?>">
                    <!-- Ticker 
                    <div class="row mb-3 align-items-center">
                        ...
                        <div class="col-sm-10">
                            <div class="input-group">
                                ...
                                <input type="text" class="form-control" id="ticker" name="ticker" value="<?php echo htmlspecialchars($ticker_cedear); ?>" readonly>
                            </div>
                        </div>
                    </div>
                    -->
                    ...
                    <div class="row mb-3 align-items-center">
                        ...
                        <div class="col-sm-10">
                            <div class="input-group">
                                ...
                                <input type="number" class="form-control" id="cantidad" name="cantidad" value="<?php echo htmlspecialchars($cantidad_cedear); ?>" autofocus required>
                            </div>
                        </div>
                    </div>
                    <!-- Precio -->
                    <div class="row mb-3 align-items-center">
                        ...
                        <div class="col-sm-10">
                            <div class="input-group">
                                ...
                                <input type="number" step="0.01" class="form-control" id="precio" name="precio" value="<?php echo htmlspecialchars($precio_cedear); ?>" required>
                            </div>
                        </div>
                    </div>
                    <!-- Fecha -->
                    <div class="row mb-3 align-items-center">
                        ...
                        <div class="col-sm-10">
                            <div class="input-group">
                                ...
                                <input type="date" class="form-control" id="fecha" name="fecha" value="<?php echo htmlspecialchars($fecha_cedear); ?>" required>
                            </div>
                        </div>
                    </div>
                    <hr class="mod mb-3">
