$(document).ready(function() {
    function updateDropdownPosition() {
        ...
        var dropdown = $('#tickerDropdown_fondos');
    ...
    $('#ticker').on('input', function() {
        ...
        if (query.length > 0) {
            $.ajax({
                url: '../funciones/tickers_fondos.php',
                type: 'GET',
                data: { term: query },
    ...
    $(document).on('click', '#tickerDropdown_fondos .dropdown-item', function(e) {
        e.preventDefault();
        var text = $(this).text().split(' - ')[0];
        $('#ticker').val(text);
        $('#tickerDropdown_fondos').empty().hide();
    });

    $('#ticker').on('keydown', function(e) {
        var dropdownItems = $('#tickerDropdown_fondos .dropdown-item');
