...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($total_compra > $saldo_en_pesos) {
        ...
        // Insertar los datos de la compra en la tabla "fondos"
        $sql_insert_fondo = "INSERT INTO fondos (cliente_id, ticker_fondos, cantidad_fondos, precio_fondos, fecha_fondos, ccl_compra) VALUES (?, ?, ?, ?, ?, ?)";
        $stmt_insert_fondo = $conn->prepare($sql_insert_fondo);
        $stmt_insert_fondo->bind_param("isidsd", $cliente_id, $ticker, $cantidad, $precio, $fecha, $promedio_ccl);
        $stmt_insert_fondo->execute();
        $stmt_insert_fondo->close();
        ...
        <div class="col-6 text-center">
            <div class="container-fluid my-4 efectivo">
                ...
                <form id="compra_fondos" method="POST" action="">
                    ...
                    <!-- Ticker -->
                    <div class="row mb-3 align-items-center">
                        <label for="ticker_fondos" class="col-sm-2 col-form-label">Ticker</label>
                        <div class="col-sm-10">
                            <div class="input-group">
                                <span class="input-group-text bg-light"><i class="fa-solid fa-chart-line"></i></span>
                                <input type="text" class="form-control" id="ticker" name="ticker" required autofocus>
                            </div>
                            <div id="tickerDropdown_fondos" class="dropdown-menu" style="display: none; width: 100%;"></div>
                        </div>
                    </div>
                    <!-- Cantidad -->
                    <div class="row mb-3 align-items-center">
