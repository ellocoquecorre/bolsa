...
$ticker_bonos = isset($_GET['ticker']) ? $_GET['ticker'] : '';
...
// Consulta para obtener los valores de cantidad, precio y id de la tabla bonos
$sql = "SELECT id, cantidad_bonos, precio_bonos, fecha_bonos FROM bonos WHERE ticker_bonos = ? AND cliente_id = ?";
$stmt = $conn->prepare($sql);
$stmt->bind_param("si", $ticker_bonos, $cliente_id);
$stmt->execute();
$stmt->bind_result($id_bono, $cantidad_bonos, $precio_bonos, $fecha_bonos);
$stmt->fetch();
$stmt->close();
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $bono_id = $_POST['bono_id'];  // Se obtiene el ID del bono
    $nuevo_cantidad = $_POST['cantidad'];
    $nuevo_precio = $_POST['precio'];
    $fecha_bonos = $_POST['fecha'];
    ...
    $precio_bonos_compra_original = $cantidad_bonos * $precio_bonos;
    if ($precio_bonos_compra_original > $nuevo_precio_compra) {
        $diferencia = $precio_bonos_compra_original - $nuevo_precio_compra;
        $nuevo_efectivo = $efectivo + $diferencia;
        $actualizar_efectivo = true;
    } else {
        $diferencia = $nuevo_precio_compra - $precio_bonos_compra_original;
    ...
    if ($actualizar_efectivo) {
        ...
        // Actualizar solo el bono específico con el ID correspondiente
        $sql = "UPDATE bonos SET cantidad_bonos = ?, precio_bonos = ?, fecha_bonos = ? WHERE id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("dssi", $nuevo_cantidad, $nuevo_precio, $fecha_bonos, $bono_id);
        $stmt->execute();
        $stmt->close();
    ...
    <div class="row mx-2 mt-navbar">
        ...
        <div class="col-6 text-center">
            <div class="container-fluid my-4 efectivo">
                <h5 class="me-2 cartera titulo-botones mb-4">Editar los datos de <?php echo htmlspecialchars($ticker_bonos); ?></h5>
                <?php if ($error_msg): ?>
                ...
                <form method="POST" action="">
                    <input type="hidden" name="bono_id" value="<?php echo htmlspecialchars($id_bono); ?>">
                    <!-- Cantidad -->
                    <div class="row mb-3 align-items-center">
                        ...
                        <div class="col-sm-10">
                            <div class="input-group">
                                ...
                                <input type="number" class="form-control" id="cantidad" name="cantidad" value="<?php echo htmlspecialchars($cantidad_bonos); ?>" autofocus required>
                            </div>
                        </div>
                    </div>
                    <!-- Precio -->
                    <div class="row mb-3 align-items-center">
                        ...
                        <div class="col-sm-10">
                            <div class="input-group">
                                ...
                                <input type="text" step="0.01" class="form-control" id="precio" name="precio" value="<?php echo htmlspecialchars($precio_bonos); ?>" placeholder="0,00" required>
                            </div>
                        </div>
                    </div>
                    <!-- Fecha -->
                    <div class="row mb-3 align-items-center">
                        ...
                        <div class="col-sm-10">
                            <div class="input-group">
                                ...
                                <input type="date" class="form-control" id="fecha" name="fecha" value="<?php echo htmlspecialchars($fecha_bonos); ?>" required>
                            </div>
                        </div>
                    </div>
                    <hr class="mod mb-3">
