...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($costo_total > $saldo_en_pesos) {
        ...
        // Obtener datos actuales de la acción
        $sql = "SELECT cantidad, precio, ccl_compra FROM acciones WHERE cliente_id = ? AND ticker = ?";
        $stmt = $conexion->prepare($sql);
        $stmt->execute([$cliente_id, $ticker]);
        $accion = $stmt->fetch();

        // Si no hay datos previos, inicializar
        $db_cantidad = $accion ? $accion['cantidad'] : 0;
        $db_precio = $accion ? $accion['precio'] : 0;
        $db_ccl = $accion ? $accion['ccl_compra'] : 0;
        ...
        // Insertar o actualizar acción
        if ($accion) {
            $sql_update = "UPDATE acciones SET cantidad = cantidad + ?, precio = ?, ccl_compra = ?, fecha = ? WHERE cliente_id = ? AND ticker = ?";
            $stmt = $conexion->prepare($sql_update);
            $stmt->execute([$cantidad_hoy, $nuevo_precio, $nuevo_ccl_compra, $fecha_hoy, $cliente_id, $ticker]);
        } else {
            $sql_insert = "INSERT INTO acciones (cliente_id, ticker, cantidad, precio, ccl_compra, fecha) VALUES (?, ?, ?, ?, ?, ?)";
